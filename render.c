...
#ifdef PSP // This kludge "solves" the PSP 512 screen widht problem
#include <psppower.h>
#endif
...
char *top_blah[] = { "www.hivelytracker.com",
                     ...
                     "this port was done by marq/fit",
                     NULL };
...
#ifdef PSP // Well, it's not 16:9 exactly...
        ptrg=sh*3;
#else
      // Ugly bodge
      if( (sh*5/4) == sw )
...
int render( void )
{
  ...
  if(vismouse)
  {
      ...
      for(y=0;y<8;y++)
        for(x=0;x<8;x++)
        {
            if(x+mouse_x>=sw || y+mouse_y>=sh)
                continue;

            if(curs[y*8+x]=='+')
                pix[(mouse_x+x)+(mouse_y+y)*512]=0;
            if(curs[y*8+x]=='.')
                pix[(mouse_x+x)+(mouse_y+y)*512]=0xffff;
        }
  }
#endif
